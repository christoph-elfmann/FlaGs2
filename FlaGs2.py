...
def run(
	...
	Entrez.email = recipients[0] #User email
	Entrez.max_tries = 5
	Entrez.sleep_between_tries = 60

	if not localGenomeList:
		if api_key:
			Entrez.api_key = api_key #Valid API-key allows 10 queries per seconds, which makes the tool run faster
	else:
		if api_key:
	...
	if redundant: #Search flanking genes in limited or all available GCFs for each query
		if redundant.isdigit():
			if int(redundant)==0:
				print('Please use -r option correctly, kindly check help or manual and try again. Thanks!')
				sys.exit()
			else:
				pass
		if not redundant.isdigit():
			if redundant.lower()=='a':
				pass
			else:
				print('Please use -r option correctly, kindly check help or manual and try again. Thanks!')
				sys.exit()
	...
	if localGenomeList:
		if localGenomeDirectory:
			if os.path.isdir(localGenomeDirectory):
				if localGenomeDirectory[-1]=='/':
					localDirIn=localGenomeDirectory
					print('Local Data path : ', localDirIn, '\n')
				else:
					localDirIn=localGenomeDirectory+'/'
					print('Local Data path : ', localDirIn, '\n')
			else:
				print('No directory Found as : '+ localGenomeDirectory)
				sys.exit()
		else:
			localDirIn='./'
	else:
	...
	if localGenomeList:
		with open (localGenomeList, 'r') as gList:
			for line in gList:
		...
		if proteinList and not assemblyList:
			with open (proteinList, 'r') as pList:
				for line in pList:
					if checkChar(line.rstrip().replace(' ',''))=='':
						Line=line.rstrip().replace(' ','').split('\t')
						if len(Line)>1:
						...
						print('The submitted query might include characters not found in NCBI protein accessions eg. > , # , ! etc. Please provide correct format, Thanks!')
						sys.exit()
		elif assemblyList and not proteinList :
			with open (assemblyList, 'r') as apList:
				for line in apList:
					if checkChar(line.rstrip().replace(' ',''))=='':
						Line=line.rstrip().replace(' ','').split('\t')
						if len(Line)<2:
	...
	if not localGenomeList:
		with open (out_prefix+'_NameError.txt', 'w') as fbad:
	...
	if localGenomeList:
		with open (out_prefix+'_Insufficient_Info_In_DB.txt', 'w') as fNai:
				...
				if queryDict[query]:
					if len(queryDict[query])!=0:
						if queryDict[query]!={'NAI'}:
							...
							if redundant:
								redun=0
								for newRed in (redundantCreate(queryDict[query],redundant)):
									redun+=1
	...
	if keep:
		with open(out_prefix+'_speciesInfo.txt','w') as asmOut:
			for query in NqueryDict:
				for item in NqueryDict[query]:
	...
	print('\t'+'Discarded protein ids with improper accession : '+str(ne)+'. See "'+out_prefix+'_NameError.txt'+'" file for details.')
	print('\t'+'Discarded protein ids lacking proper information in RefSeq DB : '+str(nai)+'. See "'+out_prefix+'_Insufficient_Info_In_DB.txt'+'" file for details.')
	...
	flankF=0
	with open (out_prefix+'_flankgene_Report.log', 'w') as errOut:
		serial=0
	...
	qcount=0
	discardedGene=0
	with open (out_prefix+'_QueryStatus.txt', 'w') as sumOut:
	...
	print('\n'+'>> Flanking Genes found : '+str(flankF)+' out of remaining '+str(serial)+'. See "'+out_prefix+'_flankgene_Report.log'+'" file for details.'+'\n'+'\n')
	...
	if tree: #Generate fasta file for making phylogenetic Tree
		with open(out_prefix+'_tree.fasta', 'w') as treeOut:
			for queries in NqueryDict:
	...
	with open(out_prefix+'_all.fasta', 'w') as all_fasta:
		for queries in NqueryDict:
			if queries in accFlankDict:
				if queries in treeFastadict:
	...
	b=0
	with open (out_prefix+'_flankgene.fasta'+'_cluster_out', 'w') as fastaNew:
		for seqids in sorted(seqDict):
			if seqDict[seqids]!='--':
				b+=1
	...
	infilename=out_prefix+'_flankgene.fasta'+'_cluster_out'

	directory = out_prefix+'_flankgene.fasta'+'_cluster_out_individuals'
	if not os.path.exists(directory):
		os.makedirs(directory)

	infile=open(out_prefix+'_flankgene.fasta'+'_cluster_out',"r").read()
	...
	if hmmdb:
		...
		if cpu:
			hmmscan_cmd = "hmmscan -E 1e-10 --cpu %s -o %s_dom.txt --domtblout %s_dom_out.txt %s %s_all.fasta"%(round(core/3), out_prefix, out_prefix, hmmdb, out_prefix)
		else:
			hmmscan_cmd = "hmmscan -E 1e-10 -o %s_dom.txt --domtblout %s_dom_out.txt %s %s_all.fasta"%(out_prefix, out_prefix, hmmdb, out_prefix)
		...
		dom_dict={}
		dom_key=''
		with open(out_prefix+"_dom_out.txt",'r') as dom_dict_infile:
	...
	with open(out_prefix+'_flankgene.fasta_cluster_out_'+iters+'_'+evthresh+'_clusters.tsv', 'r') as clusterIn:
	...
	if not tree_order:
		nPos=[]
		pPos=[]
		with open(out_prefix+'_operon.tsv', 'w') as opOut:
	...
	###matplotlib graph
		...
		# 1. Data File input
		#data = pd.read_csv(out_prefix+'_operon.tsv', sep='\t', header=None, skiprows=None)  #NOTE: Using pandas here because queries number def is missing.
		...
		main_file = open(out_prefix+'_operon.tsv','r').read()
		eg1 = main_file.split("\n\n\n\n")
		y_level_m = 0
		for m in eg1:
			if m != '':
		...
		#plt.tight_layout()
		plt.axis('off')
		plt.savefig(out_prefix+'_operon.pdf', format='pdf', bbox_inches='tight') #For FlaGs input





	if tree:###Tree Command with ETE###
		tree_file= out_prefix+'_tree.fasta'
		if cpu:
		...
		motifDict={}
		motifDict_2={}
		if os.path.isfile(out_prefix+'_tree/mafft_default-trimal01-none-fasttree_full/'+out_prefix+'_tree.fasta.final_tree.nw') == True:
			with open(out_prefix+'_tree/mafft_default-trimal01-none-fasttree_full/'+out_prefix+'_tree.fasta.final_tree.nw', 'r') as treeIn:
				for line in treeIn:
					nwTree=line
		...
		if __name__ == '__main__':
			...
			if tree_order:
				t.write(outfile=out_prefix+'_ladderTree.nw')
				t.render(out_prefix+"_flankgenes_1.svg",tree_style=ts)
			else:
				t.render(out_prefix+"_flankgenes_1.svg",tree_style=ts)
		...
		if __name__ == '__main__':
			...
			if tree_order:
				t.write(outfile=out_prefix+'_ladderTree.nw')
				t.render(out_prefix+"_flankgenes_2.svg", tree_style=ts)
			else:
				t.render(out_prefix+"_flankgenes_2.svg", tree_style=ts)

	if tree and tree_order:  # Queries in postscript file will be presented as tree order
		treeOrderList=[]
		with open(out_prefix+'_ladderTree.nw', 'r') as laddertreeIn:
	...
	###matplotlib_graph
		...
		ntPos=[]
		ptPos=[]
		with open(out_prefix+'_TreeOrder_operon.tsv', 'w') as opOut:
		...
		# 1. Data File input
		#data = pd.read_csv(out_prefix+'_TreeOrder_operon.tsv', sep='\t', header=None, skiprows=None)  #NOTE: Using pandas here because queries number def is missing.
		#df = pd.DataFrame(data)																				#Will be avoided in the FlaGs code where number queries is defined
																											   #in preceeding code.
		arrowList = []
		domainList = []
		...
		# 3. Operon (main) file input
		main_file = open(out_prefix+'_TreeOrder_operon.tsv','r').read()
		eg1 = main_file.split("\n\n\n\n")
		y_level_m = 0
		for m in eg1:
			if m != '':
		...
		#plt.tight_layout()
		plt.axis('off')
		plt.savefig(out_prefix+'_TreeOrder_output.pdf', format='pdf', bbox_inches='tight') #For FlaGs input
...
if __name__ == '__main__':
	...
	run(
		args.assemblyList,
		args.proteinList,
		args.localGenomeList,
		args.localGenomeDirectory,
		args.redundant,
		args.ethreshold,
		args.number,
		args.gene,
